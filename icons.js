(() => {
  ...
  function apply() {
    document.querySelectorAll('.quick-icon').forEach(el => {
      ...
      if (route && icons[route]) el.innerHTML = icons[route];
    });

    document.querySelectorAll('.search-wrap > span').forEach(el => { el.innerHTML = icons.search; });
    document.querySelectorAll('.file-icon').forEach(el => { el.innerHTML = icons.file; });
    document.querySelectorAll('.empty-icon').forEach(el => { el.innerHTML = icons.sparkle; });

    document.querySelectorAll('.favorite-btn').forEach(el => {
      const active = el.textContent.includes('♥');
      el.innerHTML = active ? icons.heartFilled : icons.heart;
    });

    document.querySelectorAll('[data-favorite]').forEach(el => {
      if (el.classList.contains('favorite-btn')) return;
      const text = el.textContent.trim();
      if (text.includes('избран')) {
        const active = text.includes('♥');
        el.innerHTML = `${active ? icons.heartFilled : icons.heart}<span>${active ? 'В избранном' : 'В избранное'}</span>`;
      }
    });
  }
